#!/usr/bin/env bash
# Two-stage Go2W MoE locomotion training pipeline:
#   1. Pretrain flat / rough / stairs / climb wheeled experts.
...
if [[ "${GO2W_MOE_PIPELINE_INNER:-0}" != "1" && "${FOREGROUND}" != "1" ]]; then
  if ! command -v tmux >/dev/null 2>&1; then
    echo "[ERROR] tmux not found. Re-run with FOREGROUND=1 to run in this shell." >&2
    exit 1
  fi
  TS="$(date +%Y-%m-%d_%H-%M-%S)"
  SESSION="go2w_moe_${TS}"
  LOG="${SCRIPT_DIR}/logs/go2w_moe_pipeline_${TS}.log"
  mkdir -p "${SCRIPT_DIR}/logs"
  INNER="cd '${SCRIPT_DIR}' && GO2W_MOE_PIPELINE_INNER=1 NUM_ENVS='${NUM_ENVS}'"
  ...
  INNER+=" bash scripts/train_go2w_moe.sh 2>&1 | tee '${LOG}'"
  tmux new-session -d -s "${SESSION}" "${INNER}"
  echo "[INFO] Go2W MoE pipeline launched in detached tmux session: ${SESSION}"
...
declare -A TASKS=(
  [flat]="Unitree-Go2W-Expert-Flat"
  [rough]="Unitree-Go2W-Expert-Rough"
  [stairs]="Unitree-Go2W-Expert-Stairs"
  [climb]="Unitree-Go2W-Expert-Climb"
)
declare -A EXPS=(
  [flat]="go2w_expert_flat"
  [rough]="go2w_expert_rough"
  [stairs]="go2w_expert_stairs"
  [climb]="go2w_expert_climb"
)
...
export GO2W_MOE_EXPERT_FLAT
export GO2W_MOE_EXPERT_ROUGH
export GO2W_MOE_EXPERT_STAIRS
export GO2W_MOE_EXPERT_CLIMB
GO2W_MOE_EXPERT_FLAT="$(latest_checkpoint "${EXPS[flat]}")"
GO2W_MOE_EXPERT_ROUGH="$(latest_checkpoint "${EXPS[rough]}")"
GO2W_MOE_EXPERT_STAIRS="$(latest_checkpoint "${EXPS[stairs]}")"
GO2W_MOE_EXPERT_CLIMB="$(latest_checkpoint "${EXPS[climb]}")"

echo ">>> Expert checkpoints:" >&2
echo "  flat  : ${GO2W_MOE_EXPERT_FLAT}" >&2
echo "  rough : ${GO2W_MOE_EXPERT_ROUGH}" >&2
echo "  stairs: ${GO2W_MOE_EXPERT_STAIRS}" >&2
echo "  climb : ${GO2W_MOE_EXPERT_CLIMB}" >&2

MOE_RUN="$(run_stage Unitree-Go2W-MoE-Mixed "${MOE_ITERS}" "${FIRST_GPU}")"

echo ""
echo "Go2W MoE pipeline complete."
echo "  moe: ${LOG_ROOT}/go2w_moe_mixed/${MOE_RUN}"
