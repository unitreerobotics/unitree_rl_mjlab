...
python scripts/train.py Unitree-B2YGX-Rough \
  --agent.logger=tensorboard \
  --env.scene.num-envs=4096
如果你想训练我们刚加的 actor 不带 height_scan 版本，用：

python scripts/train.py Unitree-B2YGX-Rough-NoHeightActor \
  --agent.logger=tensorboard \
  --env.scene.num-envs=4096
...
本章节数据从 `b2ygx.xml` MuJoCo 模型文件提取，代表仿真中使用的物理参数。实际硬件可能略有差异。

### 11.1 整机物理参数

| 参数 | 值 | 说明 |
|------|-----|------|
| 整机质量 | ≈87.88 kg | 包含 base + 4×(hip + thigh + calf) |
| 基座质量 | 40.8426 kg | 含雷达、摄像头等附件 |
| 基座惯性 | (0.473, 1.466, 1.708) | 对角惯性张量 (kg·m²) |
| 站立高度 | 0.546 m | 默认姿态时足端到地面距离 |

### 11.2 质量分布

| 部件 | 单件质量 (kg) | 数量 | 合计 (kg) |
|------|-------------|------|----------|
| base_link | 40.8426 | 1 | 40.84 |
| hip | 2.9294 | 4 | 11.72 |
| thigh | 7.9 | 4 | 31.6 |
| calf | 0.679 | 4 | 2.72 |
...
| 关节 | 下限 (rad) | 上限 (rad) | 下限 (°) | 上限 (°) |
|------|-----------|-----------|---------|---------|
| hip | -0.70 | +0.70 | -40.1° | +40.1° |
| thigh | -0.94 | +1.69 | -53.9° | +96.8° |
| calf | -2.62 | -0.43 | -150.1° | -24.6° |

### 11.5 执行器参数

| 关节类型 | 控制范围 | 训练 PD 增益 | 备注 |
|---------|---------|-------------|------|
| hip (×4) | ±200 Nm | kp=200, kd=10 | |
| thigh (×4) | ±200 Nm | kp=200, kd=10 | |
| calf (×4) | ±300 Nm | kp=240, kd=12 | 较大扭矩用于支撑 |
...
| 关节速度 | 12 | 编码器微分 |
| 关节力矩 | 12 | 电机力矩反馈 |
| 雷达 (Lidar) | 1 | unitree_ladar (可选) |

### 11.7 安全操作规范

1. **电机温度监控**: 长时间运行时应监控关节温度，避免过热损坏
2. **碰撞检测**: 训练和部署时均开启碰撞检测，非足端接触会导致终止
3. **姿态限制**: 部署时 `bad_orientation` 阈值为 1.0 rad (≈57°)，超过自动停机
4. **初始状态检查**: 上电后确保机器人在水平地面上初始化
5. **紧急停止**: 任何异常情况可通过切回 Passive 状态立即停止电机输出

### 11.8 维护注意事项

| 项目 | 建议周期 | 说明 |
|------|---------|------|
| 关节润滑 | 每 3 个月 | 检查并补充关节油脂 |
| 螺栓紧固 | 每 6 个月 | 检查腿部和基座连接螺栓 |
| 电缆检查 | 每 6 个月 | 检查电机和传感器线缆 |
| 软件更新 | 按需 | 关注 Unitree 官方固件更新 |
